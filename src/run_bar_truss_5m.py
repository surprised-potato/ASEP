...
def generate_building_truss_report(data_int, project_name="bar_5m"):
    ...
    report.append(f"- **Total Bays:** `4 bays` @ `{data_int['spacing']:.1f} m` spacing (Total Length: `20.0m`)")
    report.append(f"- **Main Steel Frames:** `3 Interior Trusses` (5.0m Tributary Width)")
    report.append(f"- **Gable Ends:** `2 Reinforced CHB Walls` (Bearing 2.5m Tributary Width)")
    ...
    report.append(f"\n- **Total Building Steel Weight (3 Trusses):** `{total_building_weight:,.1f} kg`")
    ...
    report.append("## 3. Global Steel Truss Cutting List (3 Trusses)")
    ...
    report.append("To ensure global stability out-of-plane for the 4-bay building, the following bracing is required:")
    ...
    report.append("- **Longitudinal Roof X-Bracing:** `16mm \phi` sag rods spanning across the roof plane in the 1st and 4th bays (adjacent to the stiff CHB gable walls).")
    report.append("- **Sidewall Vertical X-Bracing:** `20mm \phi` rods or `L2X2X1/4` angles in the 1st and 4th wall bays to transfer longitudinal shear.")
...
if __name__ == "__main__":
    ...
    data_int = run_bar_truss_optimization(spacing=5.0, span=20.0)
